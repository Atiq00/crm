    ...
    public function handle()
    {
        ...
        foreach($sales as $key => $value){  
            ...
            if(@$result->status == "Error"){
                ...
                DB::table('sap_logs')->insert(['sap_response'=>json_encode(@$result), 'sap_post_data'=>json_encode($postSapData),'status'=>3,'type'=>"CMU" ]);
            }elseif(@$result->status == "Success"){
                DB::table('sap_logs')->insert(['sap_response'=>json_encode($result), 'sap_post_data'=>json_encode($postSapData),'status'=>1,'type'=>"CMU" ]);
            }else{
                DB::table('sap_logs')->insert(['sap_response'=>json_encode($result), 'sap_post_data'=>json_encode($postSapData),'status'=>2,'type'=>"CMU" ]);
                continue;
            }
